...
const audioTracks = [
    {
        ...
        duration: "2:30",
        src: "/audio/noor.mp3"  // Update this path
    },
    {
        title: "CNoor",
        artist: "Your Favorite Song",
...
function initializeCountdown() {
    // Set default birthday to 13 August 2025 at midnight
    const birthday = new Date(2025, 7, 13, 0, 0, 0); // Month is 0-based (7 = August)
    
    const birthdayInput = document.getElementById('birthdayDate');
    if (birthdayInput) {
        birthdayInput.value = birthday.toISOString().slice(0, 16);
    }
...
function updateCountdown() {
    ...
    // If the selected date is in the past, move it to next year
    const now = new Date();
    if (selectedDate < now) {
...
function calculateTimeRemaining() {
    // Set target birthday to 13 August 2025
    const targetBirthday = new Date(2025, 7, 13, 0, 0, 0); 
    // Months in JS are 0-indexed, so 7 = August
